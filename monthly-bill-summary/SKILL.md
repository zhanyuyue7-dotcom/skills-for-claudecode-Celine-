...
description: 每月账单汇总助手。当用户提到要分析账单、处理微信/支付宝导出的 CSV/XLSX、或将消费数据整理到飞书多维表格时使用本 skill。功能包括：解析微信/支付宝账单导出文件、自动分类消费、创建飞书多维表格（含分类汇总母表+关联交易明细子表+仪表盘图表）、数据验收。仪表盘含：各分类支出对比柱图、支出占比饼图、按周消费趋势柱图。
license: MIT
---
...
[Step 1] parse_bills.py  →  parsed.json
    ↓
[Step 2] upload_to_feishu.py  →  飞书多维表格（分类汇总+交易明细+仪表盘）+ upload_state.json
    ↓
...
[Step 2.3] 创建「交易明细」表（字段：交易时间/交易对方/商品说明/金额/收支类型/关联分类/来源/交易状态/支付方式/备注/周次）
[Step 2.4] 上传分类汇总行（每个分类一条记录）
[Step 2.5] 上传全部交易明细（每50条一批，有速率限制间隔）
[Step 2.6] 创建仪表盘（3个图表：各分类支出对比柱图、支出分类占比饼图、按周消费趋势柱图）
```
...
**仪表盘说明**：脚本自动创建「仪表盘」tab，包含 3 个图表 block：
- **各分类支出对比**（柱图）：直观看各分类金额绝对值大小
- **支出分类占比**（饼图）：看各分类占总支出的比例
- **按周消费趋势**（柱图）：按第1-5周聚合，避免30天日数据拥挤

> 注意：`--base-token` 续传模式下不会重新创建仪表盘，避免重复。
